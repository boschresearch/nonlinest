...
class SampleCache:
    ...
    def get_samples(self, dimension: int, num_samples: int) -> (np.ndarray, np.ndarray):
        self.check_request(dimension, num_samples)
        ...
        if dimension not in self.data or num_samples not in self.data[dimension]:
            if not self.load_samples_from_file(dimension, num_samples):
                ...
                self.generate_sample_file(dimension, num_samples)
    ...
    def check_request(self, dimension: int, num_samples: int):
    ...
    def load_samples_from_file(self, dimension: int, num_samples: int) -> bool:
        ...
        try:
            [samples, weights] = np.load(filename, allow_pickle=True)
    ...
    def generate_sample_file(self, dimension: int, num_samples: int):
        samples, weights = self.compute_samples(dimension, num_samples)

        filename = self.get_sample_filename(dimension, num_samples)

        try:
            np.save(filename, (samples, weights), allow_pickle=True)
        except RuntimeError:
...
class SampleCacheCKF(SampleCache):
    ...
    def __init__(self):
        ...
        sample_cache_path = os.path.dirname(__file__) + "/SampleCacheCKF"
    ...
    @staticmethod
    def _compute_axes_samples(dimension: int) -> [np.ndarray, np.ndarray]:
    ...
    @staticmethod
    def _compute_off_axes_samples(dim: int) -> [np.ndarray, np.ndarray]:
        n = int(dim * (dim - 1) / 2)
        if n > 0:
            s1 = np.zeros([dim, n])
            s2 = np.zeros([dim, n])
...
class SampleCacheGHQ(SampleCache):
    ...
    def __init__(self):
        # todo package might not be installed in a writable location?
        sample_cache_path = os.path.dirname(__file__) + "/SampleCacheGHQ"
